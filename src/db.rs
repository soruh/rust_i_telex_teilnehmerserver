...
use crate::get_current_itelex_timestamp;
use crate::packages::*;
use crate::SERVER_PIN;

use crate::{CHANGED, DATABASE};
...
pub async fn read_db_from_disk() -> anyhow::Result<()> {
    ...
    if file_size % 100 != 0 {
        bail!(anyhow!(
            "DB file has length that is not a multiple of 100; It may be corrupted!"
        ));
    }
    ...
    if SERVER_PIN == 0 {
        warn!("Removing pins from read DB entries and removing private ones as to not leak them, since we are running without a SERVER_PIN");
...
pub async fn update_or_register_entry(
    package: Package1,
    ipaddress: Ipv4Addr,
) -> anyhow::Result<()> {
    // Confirm that ipaddress is not unspecified, since this could lead to entries with neither an ip nor a hostname
...
pub async fn update_entry_if_newer(entry: Package5) -> () {
    ...
    let do_update = if let Some(existing) = db.get(&entry.number) {
